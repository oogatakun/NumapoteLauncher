...
async function getNowSkin() {
    try {
        const selected = (typeof ConfigManager !== 'undefined' && ConfigManager.getSelectedAccount) ? ConfigManager.getSelectedAccount() : null
        if (!selected || !selected.uuid) {
            console.warn('getNowSkin: no selected account')
            return
        }
        const response = await axios.get(
            `https://sessionserver.mojang.com/session/minecraft/profile/${selected.uuid}`
        )
        let base64Textures = ''
...
async function getTextureID() {
    ...
    try {
        const selected = (typeof ConfigManager !== 'undefined' && ConfigManager.getSelectedAccount) ? ConfigManager.getSelectedAccount() : null
        if (!selected || !selected.uuid) {
            console.warn('getTextureID: no selected account')
            return textureID
        }
        const response = await axios.get(
            `https://sessionserver.mojang.com/session/minecraft/profile/${selected.uuid}`
...
async function uploadSkin(variant, file) {
    ...
    const selected = (typeof ConfigManager !== 'undefined' && ConfigManager.getSelectedAccount) ? ConfigManager.getSelectedAccount() : null
    if (!selected || !selected.uuid) {
        console.error('uploadSkin: no selected account')
        return
    }
    const account = ConfigManager.getAuthAccount(selected.uuid)
...
function getLauncherSkinPath() {
    ...
    switch (process.platform) {
        case 'win32':
            numapotePath = `${appPath}\\.minecraft\\numapote_skins.json`
            break
        case 'darwin':
            numapotePath = `${appPath}/minecraft/numapote_skins.json`
            break
        case 'linux':
            numapotePath = `${homePath}/.minecraft/numapote_skins.json`
            break
        default:
...
// 沼ぽてランチャー内のスキンのJSONを呼び出し・オブジェクトに変更
function loadSkins() {
...
function addSkinJSON(created, name, skinImage, modelImage, slim, textureId) {
    const jsonObject = loadSkins()
    let newIDNum = 1
    while (jsonObject['skin_numapote_' + newIDNum]) {
        newIDNum++
    }
    const id = 'skin_numapote_' + newIDNum
    jsonObject[id] = {}
...
// 初回時、自分で設定したパスでを沼ぽてランチャーにインポートする
...
// 公式と沼ぽてランチャーJSONのmerge（同期trueの時のみ動かす）
async function mergeNumapoteSkinJSON() {
    const syncSetting = checkSyncSkinJSON()
    if (syncSetting) {
        const originjsonObject = loadOriginSkins()
        const numapotejsonObject = loadSkins()
        let margedJSONObject = Object.assign(originjsonObject, numapotejsonObject)
        Object.keys(originjsonObject).forEach((key) => {
            if (numapotejsonObject[key]) {
                const originalDate = new Date(originjsonObject[key].updated)
                const numapoteDate = new Date(numapotejsonObject[key].updated)
                if (numapoteDate > originalDate) {
                    margedJSONObject[key] = numapotejsonObject[key]
                }
            }
        })
...
exports.mergeNumapoteSkinJSON = mergeNumapoteSkinJSON
