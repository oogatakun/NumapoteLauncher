...
try {
    if (customDistroLabel) {
        const distroUrl = (typeof ConfigManager !== 'undefined' && ConfigManager.getDistributionUrl) ? ConfigManager.getDistributionUrl() : ''
        customDistroLabel.innerText = distroUrl ?? ''
    }
} catch (err) {
    console.warn('Failed to set frameCustomDistro innerText', err)
}
...
// Bind launch button
const _launchButton = document.getElementById('launch_button')
if (_launchButton) {
...
const _settingsMediaButton = document.getElementById('settingsMediaButton')
...
const refreshMojangStatuses = async function(){
    ...
    const essContainer = document.getElementById('mojangStatusEssentialContainer')
    if (essContainer) essContainer.innerHTML = tooltipEssentialHTML
    const nonEssContainer = document.getElementById('mojangStatusNonEssentialContainer')
    if (nonEssContainer) nonEssContainer.innerHTML = tooltipNonEssentialHTML
    const mojangIcon = document.getElementById('mojang_status_icon')
    if (mojangIcon) mojangIcon.style.color = MojangRestAPI.statusToHex(status)
}
...
// Bind news button.
const _newsButton = document.getElementById('newsButton')
if (_newsButton) {
    _newsButton.onclick = () => {
        // Toggle tabbing.
        if(newsActive){
...
document.addEventListener('keydown', (e) => {
    if(newsActive){
        ...
        if(getCurrentView() === VIEWS.landing){
            if(e.key === 'ArrowUp'){
                const nb = document.getElementById('newsButton')
                if (nb) nb.click()
            }
        }
    }
