...
async function showMainUI(data){
    ...
    // Ensure prepareSettings is available (settings.js is loaded later in the body).
    if(typeof prepareSettings === 'function'){
        await prepareSettings(true)
    } else {
        // Wait up to 2s for settings.js to load and define prepareSettings
        const waitForPrepare = async (timeout = 2000) => {
            const start = Date.now()
            while(Date.now() - start < timeout){
                if(typeof prepareSettings === 'function') return true
                await new Promise(r => setTimeout(r, 50))
            }
            return false
        }
        const ok = await waitForPrepare(2000)
        if(ok){
            await prepareSettings(true)
        } else {
            console.warn('[UIBinder] prepareSettings not available after wait; continuing without it')
        }
    }
    const _selId = ConfigManager.getSelectedServer()
    const _selServ = data.getServerById(_selId)
    if(_selServ == null && ConfigManager.getCustomInstance(_selId) != null){
        // Custom instance selected — keep it; restore label and enable launch.
        const _ins = ConfigManager.getCustomInstance(_selId)
        const _btn = document.getElementById('server_selection_button')
        if(_btn) _btn.innerHTML = '&#8226; ' + (_ins.name || '無題の構成')
        if(typeof setLaunchEnabled === 'function') setLaunchEnabled(true)
    } else {
        updateSelectedServer(_selServ)
    }
    // refreshServerStatus()
    setTimeout(async () => {
        document.getElementById('frameBar').style.backgroundColor = 'rgba(0, 0, 0, 0.5)'
        document.body.style.backgroundImage = `url('${document.body.getAttribute('bkid')}')`

        // Apply persisted launcher overrides if present
        try {
            const overrides = ConfigManager.getLauncherOverride()
            if(overrides && Object.keys(overrides).length > 0){
                if(overrides.title){
                    const f = document.getElementById('frameTitleText')
                    if(f) f.innerText = overrides.title
                    const aboutT = document.getElementById('settingsAboutTitle')
                    if(aboutT) aboutT.innerText = overrides.title
                }
                if(overrides.background){
                    try {
                        const isListJson = typeof overrides.background === 'string' && overrides.background.toLowerCase().endsWith('/list.json')
                        if(isListJson){
                            try {
                                const res = await fetch(overrides.background, { cache: 'no-store' })
                                if(res && res.ok){
                                    const body = await res.json()
                                    let entries = []
                                    if(Array.isArray(body)) entries = body
                                    else if(body && Array.isArray(body.backgrounds)) entries = body.backgrounds
                                    if(entries.length > 0){
                                        const base = overrides.background.substring(0, overrides.background.lastIndexOf('/'))
                                        ...
                                        document.body.setAttribute('bkid', overrides.background)
                                    }
                                } else {
                                    document.body.style.backgroundImage = `url('${overrides.background}')`
                                    document.body.setAttribute('bkid', overrides.background)
                                ...
                                console.warn('Failed to fetch/parse override background list.json', err)
                                document.body.style.backgroundImage = `url('${overrides.background}')`
                                document.body.setAttribute('bkid', overrides.background)
                            }
                        } else {
                        ...
                        console.warn('Failed to apply override background', err)
                    }
                }
                if(overrides.logo){
                    ...
                    ids.forEach(id => {
                        ...
                        if(el) el.src = overrides.logo
                    })
                }
            }
        } catch (err) {
            console.warn('Failed to apply launcher overrides', err)
        }
