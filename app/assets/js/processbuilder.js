...
class ProcessBuilder {
    ...
    build(){
        ...
        this.hasModLoader = this.server.modules.some((mdl) => this._isTopLevelModLoader(mdl.rawModule.type))
        this.usingFabricLoader = this.server.modules.some(mdl => mdl.rawModule.type === Type.Fabric)
        logger.info('Using top level mod loader:', this.hasModLoader)
    ...
    _isTopLevelModLoader(type) {
        const moduleType = String(type || '')
        return moduleType === Type.ForgeHosted
            || moduleType === Type.Forge
            || moduleType === Type.Fabric
            || moduleType.toLowerCase().includes('forge')
            || moduleType.toLowerCase().includes('fabric')
    }
    ...
    _constructJVMArguments112(mods, tempNativePath){
        ...
        if(process.platform === 'darwin'){
            args.push('-Xdock:name=NumapoteLauncher')
        ...
        // Main Java Class
        args.push(this.hasModLoader ? this.modManifest.mainClass : this.vanillaManifest.mainClass)

        args = args.concat(this.hasModLoader ? this._resolveForgeArgs() : this._resolveVanillaArgs112())
    ...
    _constructJVMArguments113(mods, tempNativePath){
        ...
        if(this.hasModLoader && this.modManifest.arguments.jvm != null) {
                ...
                if(argDiscovery.test(args[i])){
                    ...
                    switch(identifier){
                        ...
                        case 'natives_directory':
                            ...
                            break
                        case 'launcher_name':
                            val = args[i].replace(argDiscovery, 'NumapoteLauncher')
                            break
    ...
    _resolveVanillaArgs112(){
        const mcArgs = this.vanillaManifest.minecraftArguments.split(' ')
        const argDiscovery = /\${*(.*)}/
    ...
    classpathArg(mods, tempNativePath){
        ...
        if(!mcVersionAtLeast('1.17', this.server.rawServer.minecraftVersion) || this.usingFabricLoader || !this.hasModLoader) {
            // Add the version.jar to the classpath.
            // Must not be added to the classpath for Forge 1.17+ (Forge provides it),
            // but vanilla (no mod loader) always needs it.
    ...
    _resolveServerLibraries(mods){
        ...
        for(let mdl of mdls){
            ...
            if(type === Type.ForgeHosted || type === Type.Fabric || type === Type.Library){
                ...
                const forgeLibs = []
                for (const library of this.modManifest.libraries) {
