...
const dataPath = path.join(sysRoot, '.numapotelauncher')

let firstLaunch = false

/**
 * Retrieve the absolute path of the launcher directory.
 * Uses @electron/remote when available, otherwise falls back to a sensible default.
 * @returns {string}
 */
exports.getLauncherDirectory = function(){
    ...
    // Fallback path similar to original default
    const sysRoot = process.env.APPDATA || (process.platform == 'darwin' ? process.env.HOME + '/Library/Application Support' : process.env.HOME)
    return path.join(sysRoot, '.numapotelauncher')
}
...
exports.getDataDirectory = function(def = false){
    if(config == null) return DEFAULT_CONFIG.settings.launcher.dataDirectory
...
// コマンドライン引数から --distro= の値を取得（renderer の process.argv を使う）
const argDistributionUrl = process.argv.find(arg => arg.startsWith('--distro='))?.split('=')[1]

// --distro= が指定されていれば優先する。distribution.txt は遅延読み込みする。
exports.getDistributionUrl = function(){
    if(argDistributionUrl) return argDistributionUrl
    if(typeof distributionUrl === 'undefined'){
        try {
            const distributionUrlConfig = path.join(exports.getLauncherDirectory(), 'distribution.txt')
            distributionUrl = fs.existsSync(distributionUrlConfig) ? fs.readFileSync(distributionUrlConfig, 'UTF-8').trim() : undefined
        } catch (err) {
            logger.warn('Failed to read distribution.txt', err)
...
/**
 * Persist a custom distribution URL to the distribution.txt file and update internal value.
 * @param {string} url The URL to persist.
 */
exports.setDistributionUrl = function(url){
    try {
        const distributionUrlConfig = path.join(exports.getLauncherDirectory(), 'distribution.txt')
        if(typeof url === 'undefined' || url === null || url === ''){
            if(fs.existsSync(distributionUrlConfig)) fs.removeSync(distributionUrlConfig)
            distributionUrl = undefined
        } else {
            fs.writeFileSync(distributionUrlConfig, String(url), 'UTF-8')
            distributionUrl = String(url)
        }
    } catch (err) {
        logger.error('Failed to write distribution.txt', err)
    }
}

/**
 * Set the distribution URL for the current runtime only (do not persist to distribution.txt).
 * Useful for temporary overrides that should not be saved across restarts.
 */
exports.setRuntimeDistributionUrl = function(url){
    try {
        if(typeof url === 'undefined' || url === null || url === ''){
            distributionUrl = undefined
        } else {
        ...
        logger.error('Failed to set runtime distribution URL', err)
    }
}

/**
 * Clear any persisted distribution URL.
 */
exports.clearDistributionUrl = function(){
    try {
        const distributionUrlConfig = path.join(exports.getLauncherDirectory(), 'distribution.txt')
        if(fs.existsSync(distributionUrlConfig)) fs.removeSync(distributionUrlConfig)
        distributionUrl = undefined
    } catch (err) {
        logger.error('Failed to clear distribution.txt', err)
    }
}

/**
 * Launcher UI overrides getter/setter/clear
 */
exports.getLauncherOverride = function(){
    return config.settings.launcher.overrides || {}
}

exports.setLauncherOverride = function(obj){
    config.settings.launcher.overrides = obj || {}
}

exports.clearLauncherOverride = function(){
    config.settings.launcher.overrides = {}
}
...
exports.load = function(){
    ...
    firstLaunch = !fs.existsSync(configPath) && !fs.existsSync(configPathLEGACY)
...
/**
 * Ensure a java config object exists for the given server id and
 * return it. If absent, create a sensible default.
 * @param {string} serverid
 * @returns {Object}
 */
function getOrCreateJavaConfig(serverid){
    if(!Object.prototype.hasOwnProperty.call(config, 'javaConfig') || config.javaConfig == null){
        config.javaConfig = {}
    }
    if(!Object.prototype.hasOwnProperty.call(config.javaConfig, serverid)){
        // Fallback to a conservative default (Java 8 style) when we don't
        // have effectiveJavaOptions available at call site.
        config.javaConfig[serverid] = defaultJavaConfig8(null)
    }
...
exports.getMinRAM = function(serverid){
    const jcfg = getOrCreateJavaConfig(serverid)
    return jcfg.minRAM
}
...
exports.getJavaExecutable = function(serverid){
    const jcfg = getOrCreateJavaConfig(serverid)
    return jcfg.customExecutable || null
}
...
exports.setJavaExecutable = function(serverid, executable){
    const jcfg = getOrCreateJavaConfig(serverid)
    jcfg.customExecutable = executable || undefined
}
...
exports.getEffectiveJavaExecutable = function(serverid){
    const jcfg = getOrCreateJavaConfig(serverid)
    return tempJavaConfig[serverid] || jcfg.customExecutable || null
}
